...
def constructBayesNet(gameState: hunters.GameState):
    ...
    fullRange=range(MAX_NOISE+X_RANGE+Y_RANGE-1) 
    variableDomainsDict[OBS0] =fullRange 
    variableDomainsDict[OBS1] =fullRange
    # raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        pNoisyGvnTrue = busters.getObservationProbability(noisyDistance, true_dist)
...
class ParticleFilter(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        for i in range(self.numParticles):
            ...
            if particle in c:
                self.particles[i] = c[particle].sample()
            else:
