...
class PacmanActionCNN(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.pool(x)
        x_res = F.relu(self.bn3(self.conv3(x)))
        x = x_res.view(x_res.size(0), -1)
        x = F.relu(self.fc1(x))
        x = F.relu(self.fc2(x))
        x = self.fc3(x)
        "*** YOUR CODE HERE ***"
...
class DQN:
    ...
    def learn(self):
        ...
        # get q-values from network
        next_q = self.network(state)
        next_q_values = self.target_network(next_state)
        
        # td_target: if terminated, only reward, otherwise reward + gamma * max(next_q)
        td_target = reward + (1 - terminated) * self.gamma * next_q_values.max(1, keepdim=True)[0]
        # compute loss with td_target and q-values
        loss = F.mse_loss(next_q.gather(1, action.long()), td_target)
