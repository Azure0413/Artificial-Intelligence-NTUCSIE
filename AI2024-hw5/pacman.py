...
def train(agent, env):
    ...
    for _ in tqdm(range(args.max_steps)):
        ...
        if agent.total_steps % args.eval_interval == 0:
            ...
            loss_hist.append(result["value_loss"])
            
            # log info to plot your figure
            "*** YOUR CODE HERE ***"
            # print("Step: {}, AvgScore: {}, Loss: {}".format(agent.total_steps, avg_score, result.get("loss", "N/A")))
            plt.plot(history['Step'], history['AvgScore'], label='Average Score')
            plt.plot(history['Step'], loss_hist, label='Value Loss')
            plt.xlabel('Step')
